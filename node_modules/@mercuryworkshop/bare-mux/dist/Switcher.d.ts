...
const transportClasses: Record<string, any> = {
  "RemoteTransport": RemoteTransport,
  // Add other transport classes if needed
};

function initTransport(name: string, config: any) {
  const TransportClass = transportClasses[name];
  if (!TransportClass) {
    throw new Error(`Transport class "${name}" not found`);
  }
  let cl = new TransportClass(...config);
